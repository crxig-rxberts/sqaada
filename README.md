...
This command starts both the server and client concurrently as well as the local DynamoDB.

### Scripts

- `npm run start`: Start the server
- `npm run start-server`: Start the server with Docker and nodemon
- `npm run client`: Start the client
- `npm run dev`: Run both server and client in development mode
- `npm run lint`: Run ESLint
- `npm run local-db`: Start local DynamoDB and create empty table
- `npm run unit-test`: (**Depends on local-db**) Run unit tests
- `npm run integration-test`:(**Depends on local-db DynamoDB**) Run integration tests
- `npm run build`: Build the client 
- `npm run docker-build`: Build the Docker image of the application
- `npm run docker-run-local`: Run the application Docker container locally
- `npm run docker-start`: Build and run the Docker container

### Docker

To build and run the application locally using Docker alongside the DB:

`npm run docker-build`
`npm run docker-run-local`

This builds the Docker image and runs it locally, with the server accessible at `http://localhost:8080`, as well as starting the local DynamoDB.
...
We adopted an Agile methodology, specifically Scrumban, for this project. This approach allowed us to:
...
2. **Planning**:
    ...
    - Collectively decided on goals and select tickets for the next sprint
    - Estimate effort for each selected item

3. **Backlog Refinement**:
    - Sessions to create and refine tickets
...
An example of this can be seen in `CreateList.test.js` or `ToDoItem.test.js`. The Component tests are lightweight, so they can be ran as part of the unit tests. 
...
1. Increase code coverage targets from 50% to 90%
...
4. Security: Spotting any security vulnerabilities
...
Our CI pipeline is triggered on every push to any branch. The pipeline consists of the following stages:
...
![testStage.png](screenshots/testStage.png)
...
We use OWASP to scan for vulnerabilities and our pipeline job has picked up on a total of 9 vulnerabilities, that need addressing in future tickets. 

The ideal state would be that the pipeline fails on *any* vulnerability and known safe vulnerabilities are to be suppressed.
...
- Add automated performance testing to ensure new changes don't negatively impact application speed
...
- Create and action tickets to remove existing OWASP vulnerabilities are resolved or suppressed where applicable.

---

# Standards Compliance

Our project adheres to several industry-recognized standards to ensure quality, accessibility, and security. Here's how we've implemented these standards:

## Accessibility Compliance (WCAG 2.1 AA)

Our application is designed to meet Web Content Accessibility Guidelines (WCAG) 2.1 AA standards, ensuring an inclusive experience for all users. Here's how we've implemented these guidelines:

### Perceivable

- **Color Contrast**: We've ensured a minimum contrast ratio of 4.5:1 for normal text and 3:1 for large text against backgrounds.
    - Example: Our primary text uses a contrast ratio of 7:1 against the background.
- **Responsive Design**: Our layout is adaptable to different screen sizes and zoom levels without loss of content or functionality.

### Operable

- **Keyboard Navigation**: All functionality is accessible via keyboard.
    - Example: Users can tab through all interactive elements and use Enter/Space to activate them.
- **Navigation**: We provide multiple ways to find content, including the Navbar and on page buttons.
...
Our API adheres to RESTful design principles:

- Used appropriate HTTP methods (GET, POST, PUT, DELETE) for CRUD operations
- Implemented consistent URL structure and naming conventions
- Utilized proper HTTP status codes for responses
- Designed stateless interactions between client and server

## Clean Code Principles

We've followed clean code principles to enhance maintainability and readability:

- Used meaningful variable and function names
- Kept functions small and focused on a single responsibility
- Implemented consistent code formatting and style

## Test-Driven Development (TDD)

We've adopted a test-driven development approach:

- Written unit tests for individual components and functions
- Implemented integration tests for API endpoints
- Used Jest and React Testing Library for frontend testing
- Plan on maintaining high test coverage to ensure code reliability
...
Our application consistently scored at or above 90 in all Lighthouse categories, but with room for improvement.

## Performance Results
To improve: Focus on optimizing JavaScript, reducing unused code, and serving images in next-gen formats.

![performance.png](screenshots/performance.png)

## Accessibility Results
To improve: Address contrast issues and manually check additional items for better inclusivity.

![accessibility.png](screenshots/accessibility.png)

## Best Practice Results
To improve: Ensure CSP effectiveness against XSS attacks and stay updated with best coding practices.

![bestpractice.png](screenshots/bestpractice.png)

## SEO Results
To improve: Add a meta description and consider additional manual checks for better search engine optimization.
...
**A screenshot of the app running in Amazon ECS:**

![awsEcs.png](screenshots/awsEcs.png)

**A screenshot of the above listId being queried in DynamoDB (listId in the url of screenshot above):**

![awsDynamodb.png](screenshots/awsDynamodb.png)

**A screenshot of the log stream:**

![awsLogs.png](screenshots/awsLogs.png)
